...
export default function BranchFormFields({ data, isView, errors = {}, clearError = () => {} }) {
  // Metadata from react-query
  const queryClient = useQueryClient();
  const welcomeData = queryClient.getQueryData(["welcome"]);
  const countryOptions = (welcomeData?.metaData?.COUNTRYCODE || []).map(c => ({ value: c.id, label: c.title }));
  const categoryOptions = (welcomeData?.metaData?.SUBCATEGORY || []).map(c => ({ value: c.id, label: c.title }));
  const idTypeOptions = (welcomeData?.metaData?.BUSINESSIDTYPE || []).map(c => ({ value: c.id, label: c.title }));
  ...
  const [mobileDial, setMobileDial] = useState("+63");
  const [businessDial, setBusinessDial] = useState("+63");
  ...
  const [country, setCountry] = useState(data?.subCountry || "");
  const [category, setCategory] = useState(data?.subCategory || "");
  const [idType, setIdType] = useState(data?.businessIdType || "");
  const [status, setStatus] = useState(data?.subStatus || "Active");
  ...
  return (
    <>
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    {dialOptions.map((countryItem) => (
                      ...
                      >
                        <Check className={cn("mr-2 h-4 w-4", mobileDial === countryItem.code ? "opacity-100" : "opacity-0")} />
                        <span className="flex items-center gap-2">
                          <span>{countryItem.code} ({countryItem.name})</span>
                        </span>
                      </CommandItem>
                    ))}
                  </CommandGroup>
        ...
        <div className="flex items-stretch w-full h-10 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 focus-within:border-[#2563eb] dark:focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-[#2563eb] dark:focus-within:ring-blue-500/20 transition-all duration-150 overflow-hidden">
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    {dialOptions.map((countryItem) => (
                      ...
                      >
                        <Check className={cn("mr-2 h-4 w-4", businessDial === countryItem.code ? "opacity-100" : "opacity-0")} />
                        <span className="flex items-center gap-2">
                          <span>{countryItem.code} ({countryItem.name})</span>
                        </span>
                      </CommandItem>
          ...
          <div className="relative flex-1 flex items-center">
            ...
            <input
              ...
              defaultValue={data?.businessPhone?.replace(businessDial, "") || ""}
              disabled={isView}
              className="w-full h-full bg-transparent border-none outline-none pl-9 pr-3 text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-650"
            />
          </div>
      ...
      <GlobalSelect
        ...
        name="subCountry"
        value={country}
        onChange={(val) => { setCountry(val); clearError("subCountry"); }}
        disabled={isView}
      ...
      <GlobalSelect
        ...
        name="subCategory"
        value={category}
        onChange={(val) => { setCategory(val); clearError("subCategory"); }}
        disabled={isView}
      ...
      <GlobalSelect
        ...
        name="businessIdType"
        value={idType}
        onChange={(val) => { setIdType(val); clearError("businessIdType"); }}
        disabled={isView}
      ...
      <GlobalSelect
        ...
        name="subStatus"
        value={status}
        onChange={(val) => { setStatus(val); clearError("subStatus"); }}
        disabled={isView}
        required
        options={[
      ...
      <div className="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
        <GlobalUpload
          ...
          onChange={async (fileBase64) => {
            ...
            try {
              const res = await uploadDocument({ imgData: fileBase64, imgType: 'proImgId' });
        ...
        <GlobalUpload
          ...
          onChange={async (fileBase64) => {
            ...
            try {
              const res = await uploadDocument({ imgData: fileBase64, imgType: 'businessIdImg' });
              if (res?.data?.[0]?.IMGIDNUM) {
