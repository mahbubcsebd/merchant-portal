...
import { formatDate, formatAmount, formatStatus } from "@/lib/utils/formatters";
...
export default function LiveTransactionsPage() {
  ...
  const {
    ...
    staleTime: 0,
    enabled: !!appliedFilters.accountId,
  });
    ...
    openDetailDialog({
      ...
      accentHeader: (
        <>
          ...
          >
            {formatStatus(tx.txnStatus, t)}
          </span>
        </>
      ),
      details: [
        { label: t("rmDateTimer", "Date"), value: formatDate(tx.creationDate) },
        { label: t("bp_ref1", "Reference No"), value: tx.confirmationNumber },
        { label: t("transaction_type", "Transaction Type"), value: tx.txnName },
        { label: t("mba_currency", "Currency Code"), value: tx.currencyCode },
      ],
    });
  };
      ...
      <div className="rounded-xl border border-slate-200 dark:border-white/8 bg-white dark:bg-white/[0.03] shadow-sm dark:shadow-none p-4 sm:p-6 min-h-[500px]">
        ...
        <div className="flex flex-col xl:flex-row gap-4 mb-8 items-stretch xl:items-end">
          <GlobalSelect
            ...
            options={[
              { value: "selected", label: t("all_statuses", "All Statuses") },
              { value: "P", label: t("rmSuccess", "Success") },
              { value: "R", label: t("global_failed", "Failed") },
              { value: "C", label: t("global_pending", "Pending") },
            ]}
          />
          <GlobalSelect
            label="Branch"
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase ${
                    tx.txnStatus === 'R' ? 'text-rose-500 dark:text-rose-400 bg-rose-50 dark:bg-rose-500/10' :
                    tx.txnStatus === 'C' ? 'text-amber-500 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10' :
                ...
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <p className="text-sm font-bold text-slate-800 dark:text-white">
                      {t("bp_ref1", "Reference No")}: {tx.confirmationNumber}
                    </p>
